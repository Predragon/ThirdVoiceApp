...
with tab2:
    ...
    with col2:
        ...
        journal['what_didnt'] = st.text_area("", value=journal['what_didnt'], 
                    ...
                    st.session_state.contacts[st.session_state.active_contact]['history'].append({
                        "id": response_id,
                        "type": result["type"],
                        "original": message if mode == "coach" else None,
                        "improved": result.get("improved") if mode == "coach" else None,
                        "interpreted": result.get("response") if mode == "translate" else None,
                        "timestamp": datetime.datetime.now().isoformat()
                    })
                    ...
                    # Feedback UI
                    st.markdown("### 📝 Please rate this response:")
                    feedback_col1, feedback_col2, feedback_col3 = st.columns(3)
                    with feedback_col1:
                        if st.button("👍 Helpful", key=f"feedback_pos_{response_id}"):
                            collect_feedback(response_id, result["type"], rating=1)
                            st.success("Thanks for your feedback! 👍")
                    with feedback_col2:
                        if st.button("👎 Not Helpful", key=f"feedback_neg_{response_id}"):
                            collect_feedback(response_id, result["type"], rating=0)
                            st.warning("Thanks for your feedback! We'll improve.")
                    with feedback_col3:
                        comment = st.text_input("Additional comments", key=f"feedback_comment_{response_id}")
                        if comment:
                            collect_feedback(response_id, result["type"], rating=None, comment=comment)
                            st.info("Comment saved. Thank you!")

                else:
                    st.error(result["error"])

# Journal section for tracking communication wins
st.markdown("---")
st.markdown("## 📝 Communication Journal")
if st.session_state.journal_entries.get(st.session_state.active_contact):
    for entry in st.session_state.journal_entries[st.session_state.active_contact]:
        dt = entry.get("timestamp", "")[:19].replace("T", " ")
        st.markdown(f"- **{dt}**: {entry.get('note', '')}")
else:
    st.markdown("_No journal entries yet. You can add notes below._")

journal_note = st.text_area("Add a journal note about your communication:", height=80)
if st.button("💾 Save Journal Note"):
    contact_journal = st.session_state.journal_entries.setdefault(st.session_state.active_contact, [])
    contact_journal.append({
        "note": journal_note,
        "timestamp": datetime.datetime.now().isoformat()
    })
