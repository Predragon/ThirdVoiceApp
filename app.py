...
st.sidebar.markdown("---") st.sidebar.markdown("### 📜 Manage History")

uploaded = st.sidebar.file_uploader("📤 Load History", type="json") if uploaded: try: data = json.load(uploaded) if isinstance(data, list): st.session_state.history = data st.sidebar.success("✅ History loaded!") except: st.sidebar.error("❌ Invalid file")
...
with tab1: st.markdown("### ✍️ Improve Message") msg = st.text_area("Your message:", value=st.session_state.active_msg, height=80, key="coach_msg") if st.button("🚀 Improve", type="primary"): st.session_state.count += 1 result = analyze_with_openrouter(msg, st.session_state.active_ctx) sentiment = result.get("sentiment", "neutral") st.markdown(f'<div class="{sentiment[:3]}">{sentiment.title()} • {result.get("emotion", "neutral").title()}</div>', unsafe_allow_html=True) improved = result.get("reframed", msg) st.markdown(f'<div class="ai-box">{improved}</div>', unsafe_allow_html=True) st.session_state.history.append({ "time": datetime.datetime.now().strftime("%m/%d %H:%M"), "type": "send", "context": st.session_state.active_ctx, "original": msg, "result": improved, "sentiment": sentiment }) st.code(improved, language="text")

with tab2: st.markdown("### 🧠 Understand Received Message") msg = st.text_area("Received message:", value=st.session_state.active_msg, height=80, key="translate_msg") if st.button("🔍 Analyze", type="primary"): st.session_state.count += 1 result = analyze_with_openrouter(msg, st.session_state.active_ctx, True) sentiment = result.get("sentiment", "neutral") st.markdown(f'<div class="{sentiment[:3]}">{sentiment.title()} • {result.get("emotion", "neutral").title()}</div>', unsafe_allow_html=True) st.markdown(f"Meaning: {result.get('meaning', '...')}") st.markdown(f"Need: {result.get('need', '...')}") st.markdown(f'<div class="ai-box">{result.get("response", "I understand.")}</div>', unsafe_allow_html=True) st.session_state.history.append({ "time": datetime.datetime.now().strftime("%m/%d %H:%M"), "type": "receive", "context": st.session_state.active_ctx, "original": msg, "result": result.get("response", msg), "sentiment": sentiment }) st.code(result.get("response", msg), language="text")

with tab3: st.markdown("### 📜 Conversation History") filter_ctx = st.selectbox("Filter by context", CONTEXTS, index=CONTEXTS.index(st.session_state.active_ctx), key="history_filter") filtered = [h for h in st.session_state.history if h['context'] == filter_ctx] if not filtered: st.info("No messages yet for this context.") else: for i, entry in enumerate(reversed(filtered)): st.markdown(f"#### Message #{len(filtered) - i}") st.markdown(f"- 🕒 {entry['time']}") st.markdown(f"- 💬 Type: {entry['type']}") st.markdown(f"- 😊 Sentiment: {entry['sentiment']}") st.markdown(f"- ✉️ Original: {entry['original']}") box_class = "sent-box" if entry["type"] == "send" else "received-box" box_label = "🟢 You said:" if entry["type"] == "send" else "🔵 You received:" st.markdown(f"<div class='{box_class}'><strong>{box_label}</strong><br>{entry['result']}</div>", unsafe_allow_html=True) st.markdown("---")

with tab4: st.markdown("### 📘 Reflection Journal") st.text_area("What worked well in this session?", key="journal", height=150) if st.button("💾 Save Reflection"): st.success("Reflection saved (locally in session)")

with tab5: st.markdown("""### ℹ️ About The Third Voice AI communication coach for better conversations.
